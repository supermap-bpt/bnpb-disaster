...
import type { Bbox, FloodOverlay } from "@/components/FloodMap";
...
import { CheckSquare, Check, Circle, Download, Eye, EyeOff, Info, Loader2, MapPin, MapPinOff, RefreshCw, Square, Trash2, Waves } from "lucide-react";
...
const FLOOD_STATUS_LABEL_KEY: Record<string, TranslationKey> = {
  pending: "floodStatusPending",
  processing: "floodStatusProcessing",
  completed: "floodStatusCompleted",
  failed: "floodStatusFailed",
};

const FLOOD_STAGE_KEYS: TranslationKey[] = [
  "floodStage1",
  "floodStage2",
  "floodStage3",
  "floodStage4",
...
const FLOOD_STATUS_BADGE_CLASS: Record<string, string> = {
...
function FloodPage() {
  ...
  const [jobs, setJobs] = useState<FloodJob[]>([]);
  ...
  const handleTogglePreview = async (jobId: string) => {
    ...
    try {
      const { bounds } = await fetchFloodPreview(jobId);
      const [south, west, north, east] = bounds;
      setFloodOverlay({
        url: getFloodPreviewImageUrl(jobId),
        bounds: [
          [south, west],
          [north, east],
        ],
      ...
      setError(err instanceof Error ? err.message : t("floodPreviewFailed"));
    }
  };
  ...
  const loadJobs = async () => {
    try {
      const { items } = await fetchFloodJobs();
      setJobs(items);
    } catch {
  ...
  const handleProcessFlood = async () => {
    if (selectedId === null) {
      setError(t("floodNeedOne"));
      return;
    }
    // No AOI = full ~250 km scene = very slow (~20 min, GBs). Make the user confirm.
    if (!bbox && !window.confirm(t("floodNoAoiConfirm"))) {
      return;
    }
    setIsProcessing(true);
    setError(null);
    try {
      await processFlood(selectedId, bbox ?? undefined);
      setSelectedId(null);
      await loadJobs();
    } catch (err) {
      setError(err instanceof Error ? err.message : t("floodStartFailed"));
    } finally {
  ...
  const handleDeleteJob = async (id: string) => {
    if (!window.confirm(t("floodDeleteJobConfirm"))) return;
    try {
      await deleteFloodJob(id);
      await loadJobs();
    } catch (err) {
      setError(err instanceof Error ? err.message : t("floodStartFailed"));
    }
    ...
    <main className="flex flex-1 gap-3 overflow-hidden p-3">
      <div className="flex w-1/2 flex-col overflow-hidden rounded-xl border bg-background shadow-sm">
        <div className="flex flex-col gap-3 border-b p-4">
          <h1 className="text-xl font-semibold">{t("floodSectionTitle")}</h1>
          ...
          <div className="flex flex-col gap-2 rounded-lg border border-dashed bg-muted/30 p-2">
            <div className="flex items-center gap-2">
              ...
              <span className="shrink-0 text-[11px] text-muted-foreground">
                {selectedId === null ? 0 : 1}/1
              </span>
              <Button
                type="button"
                size="sm"
                disabled={selectedId === null || isProcessing}
                onClick={handleProcessFlood}
                className="h-7 shrink-0 px-2 text-[11px]"
              >
                <Waves className="h-3 w-3" /> {t("floodProcess")}
              </Button>
            </div>
          </div>
        ...
        <div className="flex flex-1 flex-col gap-3 overflow-y-auto p-4">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-1.5">
                    ...
                    {item.fileStatus === "completed" && (
                      <Button
                        ...
                        disabled={selectedId !== item.id && selectedId !== null}
                        className="h-7 px-2 text-[11px]"
                      >
                        {selectedId === item.id ? (
                          <>
                            <CheckSquare className="h-3 w-3" /> {t("floodSelected")}
                          </>
                        ) : (
                          <>
                            <Square className="h-3 w-3" /> {t("floodSelect")}
                          </>
                        )}
                      </Button>
                    )}
          ...
          {jobs.length > 0 && (
            <div className="mt-2 flex flex-col gap-2 border-t pt-3">
              <h2 className="text-sm font-semibold">{t("floodJobsTitle")}</h2>
              {jobs.map((job) => (
                <Card key={job.id} className="rounded-lg p-3 shadow-sm">
                  <CardContent className="p-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span
                        className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] ${FLOOD_STATUS_BADGE_CLASS[job.status] ?? ""}`}
                      >
                        {t(FLOOD_STATUS_LABEL_KEY[job.status] ?? "floodStatusPending")}
                      </span>
                    </div>
                      ...
                      {job.hasResult ? (
                        <>
                          ...
                          >
                            {previewJobId === job.id ? (
                              <>
                                <EyeOff className="h-3 w-3" /> {t("floodHidePreview")}
                              </>
                            ) : (
                              <>
                                <Eye className="h-3 w-3" /> {t("floodPreview")}
                              </>
                            )}
                          </Button>
                          <Button asChild type="button" size="sm" variant="outline" className="h-7 flex-1 px-2 text-[11px]">
                            <a href={getFloodResultUrl(job.id)} download>
                              <Download className="h-3 w-3" /> {t("floodDownloadResult")}
                            </a>
                          </Button>
                          <Button asChild type="button" size="sm" variant="outline" className="h-7 flex-1 px-2 text-[11px]">
                            <a href={getFloodKmzUrl(job.id)} download>
                              <Download className="h-3 w-3" /> {t("floodDownloadKmz")}
                            </a>
                          </Button>
                        </>
                      ) : (
